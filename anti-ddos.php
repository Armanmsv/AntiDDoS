 ...
 * @version 1.0
 * @package ArmanMsv
 * @author Arman Msv (armanmsv06@gmail.com)
 * @copyright (C) CleanTalk team (https://armanmsv.ir)
 *
**/

include "./src/config.php" ; //Do not touch

# Switch to control AntiDDoS state.
$anti_ddos_protection_enable = $AM_anti_ddos_protection_enable ;
# Activate debug statements.
$anti_ddos_debug = $AM_anti_ddos_debug ;
# Test visitors against trusted UserAgent's list.
$test_not_rated_ua = $AM_test_not_rated_ua ;
...
if ($anti_ddos_protection_enable && isset($_SERVER['REMOTE_ADDR'])) {
    ...
    $secure_cookie_label = $AM_secure_cookie_label ;
    ...
    if ($run_stop_action) {
        $html_file = file_get_contents(dirname(__FILE__) . './src/anti-ddos.html');
